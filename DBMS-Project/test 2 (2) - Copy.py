    ...
    if signinsuccess:
        ...
        def add():
            ...
            def addbook():
                ...
                stock=main.dboutofstock(cursor,username,book_name)
                if stock<3:
                    main.dbaddbook(cursor,username,book_name,maxchapters,chapters,rating)
                    connection.commit()
                else:
                    messagebox.showerror("Invalid", "{} out of stock".format(book_name))
                screen.destroy()
                watchlist.deiconify()
...
##########################################################
        ...
        b3=Button(frame,image=img3,borderwidth=0,highlightthickness=0,command=logout,relief="flat")
        b3.place(x=870,y=20,width=35,height=32)
        #value=['','','','']
        ...
        profile = Label(frame, text=username, fg='black', bg='#3D708D', font=('JosefinSansRoman-Regular', 14, 'bold'))
        profile.place(x=750, y=20)
        # canvas.create_text(
        #     859.5, 30.0,
        #     text="USERNAME",
        #     fill="#ffffff",
...
Button(frame, width=39, pady=7, text='Sign in',fg="white", bg='#652d90', border=0, command=signin).place(x=35, y=204)
